...
var debug = function debug() {
  ...
  args = ['Auth: ' + arguments[0]].concat(args.slice(1, args.length));
...
var getAuthForSessionToken = function getAuthForSessionToken() {
  ...
  return config.cacheController.user.get(sessionToken).then(function (userJSON) {
    if (userJSON) {
      var cachedUser = Parse.Object.fromJSON(JSON.parse(userJSON));
    ...
    return query.execute().then(function (response) {
      ...
      config.cacheController.user.put(sessionToken, JSON.stringify(obj));
