  ...
  return this.loadSchema().then(function (schemaController) {
    ...
    return schemaController.getOneSchema(className, isMaster).catch(function (error) {
      ...
      return (isMaster ? Promise.resolve() : schemaController.validatePermission(className, aclGroup, op)).then(function () {
        if (typeof _this10.adapter.leftJoin === 'function' && query['$relatedTo']) {
          // do the thing
          return;
        }
        ...
        if (count && !relatedTo) {
          ...
          var promise = void 0;
          if (relatedTo) {
            var joinTable = joinTableName(relatedTo.object.className, relatedTo.key);
            promise = _this10.adapter.leftJoin(joinTable, 'relatedId', className, '_id', schema, {
              'owningId': relatedTo.object.objectId
            }, query, { skip: skip, limit: limit, sort: sort, keys: keys, readPreference: readPreference, count: count });
          } else {
            promise = _this10.adapter.find(className, schema, query, { skip: skip, limit: limit, sort: sort, keys: keys, readPreference: readPreference });
          }
