...
DatabaseController.prototype.relatedIds = function (className, key, owningId, queryOptions) {
  ...
  return this.adapter.find(joinTableName(className, key), relationSchema, { owningId: owningId }, findOptions).then(function (results) {
...
DatabaseController.prototype.reduceRelationKeys = function (className, query, queryOptions) {
  ...
  if (query['$or']) {
    return Promise.all(query['$or'].map(function (aQuery) {
      return _this9.reduceRelationKeys(className, aQuery, queryOptions);
    }));
  }

  var relatedTo = query['$relatedTo'];
  if (relatedTo) {
    return this.relatedIds(relatedTo.object.className, relatedTo.key, relatedTo.object.objectId, queryOptions).then(function (ids) {
      delete query['$relatedTo'];
      _this9.addInObjectIdsIds(ids, query);
      return _this9.reduceRelationKeys(className, query, queryOptions);
    });
  }
};
...
DatabaseController.prototype.find = function (className, query) {
  ...
  return this.loadSchema().then(function (schemaController) {
    ...
    return schemaController.getOneSchema(className, isMaster).catch(function (error) {
      ...
      var queryOptions = { skip: skip, limit: limit, sort: sort, keys: keys, readPreference: readPreference };
      return (isMaster ? Promise.resolve() : schemaController.validatePermission(className, aclGroup, op)).then(function () {
        return _this10.reduceRelationKeys(className, query, queryOptions);
      }).then(function () {
        ...
        if (count) {
          ...
          if (!classExists) {
            ...
            return _this10.adapter.distinct(className, schema, query, distinct);
          }
        } else if (pipeline) {
          if (!classExists) {
            return [];
          } else {
            return _this10.adapter.aggregate(className, pipeline, readPreference);
          }
        } else {
          if (!classExists) {
            return [];
          } else {
            return _this10.adapter.find(className, schema, query, queryOptions).then(function (objects) {
